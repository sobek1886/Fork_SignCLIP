        ...
        if "predict" not in self.job_type:
            ...
            for field in config.fairseq:
                for key in config.fairseq[field]:
                    if key in ["fp16", "reset_optimizer", "reset_lr_scheduler", "reset_dataloader", "reset_meters", "no_epoch_checkpoints", "no_last_checkpoints", "disable_validation"]:  # a list of binary flag.
                        param = ["--" + key.replace("_", "-")]
                    else:
                        if key == "lr":
